                ...
                {points.map((point) => (
                  <TableRow key={point.id}>
                    ...
                    <TableCell align="center">
                      {point.station ? point.station.name : <b>-</b>}
                    </TableCell>
                    <TableCell align="center">
                      {' '}
                      {point.active ? (
                        <Switch
                          defaultValue={point || false}
                          value={point.active ?? ' '}
                          defaultChecked
                          onClick={() => updatePoint(point.id, !point.active)}
                        />
                      ) : (
                        <Switch
                          defaultValue={point || false}
                    ...
                    <TableCell align="center">
                      <FormControl fullWidth>
                        ...
                        <Select
                          onChange={(e) =>
                            updatePointsBike(point.id, e.target.value, point.id)
                          }
                        ...
                        >
                          ...
                          {bikes
                            ...
                            .map((bike) => (
                              <MenuItem
                                value={parseInt(bike.id)}
                                key={`${point.id}bike${bike.id}`}
                              >
                                {`Bici ${bike.id}; ${
                                  bike.active ? 'Activa' : 'Deshabilitada'
                                }; ${
                                  bike.points
                                    ? `Punto ${bike.points.id}`
                                    : 'En ningún punto o en uso'
                                }`}
                              </MenuItem>
                            ))}
                        </Select>
